...
-- Usa INSERT IGNORE y NOT EXISTS para omitir datos que ya existen.
-- Incluye ajustes de esquema (precision decimal y clave unica).
-- ============================================

-- ============================================
-- 0. PREPARACION: Eliminar FK constraints de Script 1 que referencian
--    la tabla 'estudiantes' (plural). Hibernate usa la tabla 'estudiante'
--    (singular), por lo que estas FK impiden insertar datos correctamente.
-- ============================================
SET @fk_beca = (SELECT COUNT(*) FROM information_schema.TABLE_CONSTRAINTS
    WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'becas'
    AND CONSTRAINT_TYPE = 'FOREIGN KEY' AND CONSTRAINT_NAME = 'fk_estudiante_beca');
SET @sql_fk1 = IF(@fk_beca > 0, 'ALTER TABLE becas DROP FOREIGN KEY fk_estudiante_beca', 'SELECT 1');
PREPARE stmt_fk1 FROM @sql_fk1;
EXECUTE stmt_fk1;
DEALLOCATE PREPARE stmt_fk1;

SET @fk_desc = (SELECT COUNT(*) FROM information_schema.TABLE_CONSTRAINTS
    WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'descuentos'
    AND CONSTRAINT_TYPE = 'FOREIGN KEY' AND CONSTRAINT_NAME = 'fk_estudiante_descuento');
SET @sql_fk2 = IF(@fk_desc > 0, 'ALTER TABLE descuentos DROP FOREIGN KEY fk_estudiante_descuento', 'SELECT 1');
PREPARE stmt_fk2 FROM @sql_fk2;
EXECUTE stmt_fk2;
DEALLOCATE PREPARE stmt_fk2;

-- ============================================
-- 0.1 UNIQUE CONSTRAINTS para idempotencia de INSERT IGNORE
-- ============================================

-- configuracion_reporte_financiero: una config por periodo
DELETE crf1 FROM configuracion_reporte_financiero crf1
INNER JOIN configuracion_reporte_financiero crf2
ON crf1.periodo_academico_id = crf2.periodo_academico_id AND crf1.id > crf2.id;

SET @uk_crf = (SELECT COUNT(*) FROM information_schema.TABLE_CONSTRAINTS
    WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'configuracion_reporte_financiero'
    AND CONSTRAINT_TYPE = 'UNIQUE' AND CONSTRAINT_NAME = 'uk_config_rf_periodo');
SET @sql_crf = IF(@uk_crf = 0,
    'ALTER TABLE configuracion_reporte_financiero ADD UNIQUE KEY uk_config_rf_periodo (periodo_academico_id)',
    'SELECT 1');
PREPARE stmt_crf FROM @sql_crf;
EXECUTE stmt_crf;
DEALLOCATE PREPARE stmt_crf;

-- configuracion_reporte_grupos: una config por periodo
DELETE crg1 FROM configuracion_reporte_grupos crg1
INNER JOIN configuracion_reporte_grupos crg2
ON crg1.periodo_academico_id = crg2.periodo_academico_id AND crg1.id > crg2.id;

SET @uk_crg = (SELECT COUNT(*) FROM information_schema.TABLE_CONSTRAINTS
    WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'configuracion_reporte_grupos'
    AND CONSTRAINT_TYPE = 'UNIQUE' AND CONSTRAINT_NAME = 'uk_config_rg_periodo');
SET @sql_crg = IF(@uk_crg = 0,
    'ALTER TABLE configuracion_reporte_grupos ADD UNIQUE KEY uk_config_rg_periodo (periodo_academico_id)',
    'SELECT 1');
PREPARE stmt_crg FROM @sql_crg;
EXECUTE stmt_crg;
DEALLOCATE PREPARE stmt_crg;

-- matricula_financiera: una matricula por estudiante por periodo
DELETE mf1 FROM matricula_financiera mf1
INNER JOIN matricula_financiera mf2
ON mf1.periodo_academico_id = mf2.periodo_academico_id
AND mf1.id_estudiante = mf2.id_estudiante AND mf1.id > mf2.id;

SET @uk_mf = (SELECT COUNT(*) FROM information_schema.TABLE_CONSTRAINTS
    WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'matricula_financiera'
    AND CONSTRAINT_TYPE = 'UNIQUE' AND CONSTRAINT_NAME = 'uk_mf_periodo_est');
SET @sql_mf = IF(@uk_mf = 0,
    'ALTER TABLE matricula_financiera ADD UNIQUE KEY uk_mf_periodo_est (periodo_academico_id, id_estudiante)',
    'SELECT 1');
PREPARE stmt_mf FROM @sql_mf;
EXECUTE stmt_mf;
DEALLOCATE PREPARE stmt_mf;

-- gasto_general: un gasto por descripcion por configuracion
DELETE gg1 FROM gasto_general gg1
INNER JOIN gasto_general gg2
ON gg1.descripcion = gg2.descripcion
AND gg1.configuracion_reporte_grupos_id = gg2.configuracion_reporte_grupos_id AND gg1.id_gasto_general > gg2.id_gasto_general;

SET @uk_gg = (SELECT COUNT(*) FROM information_schema.TABLE_CONSTRAINTS
    WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'gasto_general'
    AND CONSTRAINT_TYPE = 'UNIQUE' AND CONSTRAINT_NAME = 'uk_gasto_desc_config');
SET @sql_gg = IF(@uk_gg = 0,
    'ALTER TABLE gasto_general ADD UNIQUE KEY uk_gasto_desc_config (descripcion, configuracion_reporte_grupos_id)',
    'SELECT 1');
PREPARE stmt_gg FROM @sql_gg;
EXECUTE stmt_gg;
DEALLOCATE PREPARE stmt_gg;
...
INSERT IGNORE INTO matricula_financiera (fecha_matricula, valor_matricula, pagada, periodo_academico_id, id_estudiante) VALUES
('2020-01-15', 5000000.00, TRUE, 1, 1),
('2020-01-16', 5000000.00, TRUE, 1, 2),
('2020-01-17', 5000000.00, FALSE, 1, 5),
('2020-01-18', 5000000.00, TRUE, 1, 6),
('2020-01-19', 5000000.00, TRUE, 1, 9),
('2020-01-20', 5000000.00, FALSE, 1, 3),
('2020-01-21', 5000000.00, TRUE, 1, 4),
('2020-01-22', 5000000.00, TRUE, 1, 7),
('2020-01-23', 5000000.00, FALSE, 1, 8),
('2020-01-24', 5000000.00, TRUE, 1, 10),
-- Período 2-2020 (ID 2)
('2020-07-10', 5200000.00, TRUE, 2, 1),
('2020-07-11', 5200000.00, FALSE, 2, 2),
('2020-07-12', 5200000.00, TRUE, 2, 3),
('2020-07-13', 5200000.00, TRUE, 2, 4),
('2020-07-14', 5200000.00, FALSE, 2, 5),
('2020-07-15', 5200000.00, TRUE, 2, 6),
('2020-07-16', 5200000.00, TRUE, 2, 7),
('2020-07-17', 5200000.00, FALSE, 2, 8),
('2020-07-18', 5200000.00, TRUE, 2, 9),
('2020-07-19', 5200000.00, TRUE, 2, 10),
-- Período 1-2021 (ID 3)
('2021-01-18', 5500000.00, TRUE, 3, 11),
('2021-01-19', 5500000.00, TRUE, 3, 12),
('2021-01-20', 5500000.00, FALSE, 3, 13),
('2021-01-21', 5500000.00, TRUE, 3, 14),
('2021-01-22', 5500000.00, TRUE, 3, 15),
('2021-01-23', 5500000.00, FALSE, 3, 16),
('2021-01-24', 5500000.00, TRUE, 3, 17),
('2021-01-25', 5500000.00, TRUE, 3, 18),
('2021-01-26', 5500000.00, FALSE, 3, 19),
('2021-01-27', 5500000.00, TRUE, 3, 20),
-- Período 2-2021 (ID 4)
('2021-07-12', 5700000.00, TRUE, 4, 11),
('2021-07-13', 5700000.00, FALSE, 4, 12),
('2021-07-14', 5700000.00, TRUE, 4, 13),
('2021-07-15', 5700000.00, TRUE, 4, 14),
('2021-07-16', 5700000.00, FALSE, 4, 15),
('2021-07-17', 5700000.00, TRUE, 4, 16),
('2021-07-18', 5700000.00, TRUE, 4, 17),
('2021-07-19', 5700000.00, FALSE, 4, 18),
('2021-07-20', 5700000.00, TRUE, 4, 19),
('2021-07-21', 5700000.00, TRUE, 4, 20),
-- Período 1-2022 (ID 5)
('2022-01-16', 6000000.00, TRUE, 5, 21),
('2022-01-17', 6000000.00, TRUE, 5, 22),
('2022-01-18', 6000000.00, FALSE, 5, 23),
('2022-01-19', 6000000.00, TRUE, 5, 24),
('2022-01-20', 6000000.00, TRUE, 5, 25),
('2022-01-21', 6000000.00, FALSE, 5, 26),
('2022-01-22', 6000000.00, TRUE, 5, 27),
('2022-01-23', 6000000.00, TRUE, 5, 28),
('2022-01-24', 6000000.00, FALSE, 5, 29),
('2022-01-25', 6000000.00, TRUE, 5, 30),
-- Período 2-2022 (ID 6)
('2022-07-14', 6200000.00, TRUE, 6, 21),
('2022-07-15', 6200000.00, FALSE, 6, 22),
('2022-07-16', 6200000.00, TRUE, 6, 23),
('2022-07-17', 6200000.00, TRUE, 6, 24),
('2022-07-18', 6200000.00, FALSE, 6, 25),
('2022-07-19', 6200000.00, TRUE, 6, 26),
('2022-07-20', 6200000.00, TRUE, 6, 27),
('2022-07-21', 6200000.00, FALSE, 6, 28),
('2022-07-22', 6200000.00, TRUE, 6, 29),
('2022-07-23', 6200000.00, TRUE, 6, 30),
-- Período 1-2023 (ID 7)
('2023-01-20', 6500000.00, TRUE, 7, 31),
('2023-01-21', 6500000.00, TRUE, 7, 32),
('2023-01-22', 6500000.00, FALSE, 7, 33),
('2023-01-23', 6500000.00, TRUE, 7, 34),
('2023-01-24', 6500000.00, TRUE, 7, 35),
('2023-01-25', 6500000.00, FALSE, 7, 36),
('2023-01-26', 6500000.00, TRUE, 7, 37),
('2023-01-27', 6500000.00, TRUE, 7, 38),
('2023-01-28', 6500000.00, FALSE, 7, 39),
('2023-01-29', 6500000.00, TRUE, 7, 40),
-- Período 2-2023 (ID 8)
('2023-07-15', 6700000.00, TRUE, 8, 31),
('2023-07-16', 6700000.00, FALSE, 8, 32),
('2023-07-17', 6700000.00, TRUE, 8, 33),
('2023-07-18', 6700000.00, TRUE, 8, 34),
('2023-07-19', 6700000.00, FALSE, 8, 35),
('2023-07-20', 6700000.00, TRUE, 8, 36),
('2023-07-21', 6700000.00, TRUE, 8, 37),
('2023-07-22', 6700000.00, FALSE, 8, 38),
('2023-07-23', 6700000.00, TRUE, 8, 39),
('2023-07-24', 6700000.00, TRUE, 8, 40),
-- Período 1-2024 (ID 9)
('2024-01-15', 7000000.00, TRUE, 9, 41),
('2024-01-16', 7000000.00, TRUE, 9, 42),
('2024-01-17', 7000000.00, FALSE, 9, 45),
('2024-01-18', 7000000.00, TRUE, 9, 46),
('2024-01-19', 7000000.00, TRUE, 9, 49),
('2024-01-20', 7000000.00, FALSE, 9, 1),
('2024-01-21', 7000000.00, TRUE, 9, 2),
('2024-01-22', 7000000.00, TRUE, 9, 5),
('2024-01-23', 7000000.00, FALSE, 9, 6),
('2024-01-24', 7000000.00, TRUE, 9, 9),
-- Período 2-2024 (ID 10) - periodo actual (activo)
('2024-07-10', 7200000.00, TRUE, 10, 43),
('2024-07-11', 7200000.00, FALSE, 10, 44),
('2024-07-12', 7200000.00, TRUE, 10, 47),
('2024-07-13', 7200000.00, TRUE, 10, 48),
('2024-07-14', 7200000.00, FALSE, 10, 50),
('2024-07-15', 7200000.00, TRUE, 10, 3),
('2024-07-16', 7200000.00, TRUE, 10, 4),
('2024-07-17', 7200000.00, FALSE, 10, 7),
('2024-07-18', 7200000.00, TRUE, 10, 8),
('2024-07-19', 7200000.00, TRUE, 10, 10);

-- 9. DESCUENTOS (Depende de estudiante). Solo id_estudiante 1 a 10 (persona_id).
-- Usa LEFT JOIN para omitir filas que ya existen (un descuento por estudiante en datos de prueba).
INSERT INTO descuentos (id_estudiante, fechainiciodes, fechafindes, tipodes, porcentajedes, numactades, fechaactades, numresoldes, resoluciondes, poliza, estado)
SELECT t.id_estudiante, t.fechainiciodes, t.fechafindes, t.tipodes, t.porcentajedes, t.numactades, t.fechaactades, t.numresoldes, t.resoluciondes, t.poliza, t.estado
FROM (
    SELECT 1 AS id_estudiante, '2020-01-01' AS fechainiciodes, '2020-12-31' AS fechafindes, 'Beca Académica' AS tipodes, 10 AS porcentajedes, 'ACTA-001' AS numactades, '2020-01-10' AS fechaactades, 'RES-001' AS numresoldes, 'RES-2020-001' AS resoluciondes, 'POL-2020-001' AS poliza, TRUE AS estado
    UNION ALL SELECT 2, '2020-01-01', '2020-12-31', 'Beca Deportiva', 15, 'ACTA-002', '2020-01-12', 'RES-002', 'RES-2020-002', 'POL-2020-002', TRUE
    UNION ALL SELECT 3, '2020-07-01', '2021-06-30', 'Beca Socioeconómica', 20, 'ACTA-003', '2020-07-05', 'RES-003', 'RES-2020-003', 'POL-2020-003', TRUE
    UNION ALL SELECT 4, '2021-01-01', '2021-12-31', 'Beca Académica', 10, 'ACTA-004', '2021-01-08', 'RES-004', 'RES-2021-001', 'POL-2021-001', TRUE
    UNION ALL SELECT 5, '2021-01-01', '2021-12-31', 'Beca Cultural', 5, 'ACTA-005', '2021-01-15', 'RES-005', 'RES-2021-002', 'POL-2021-002', TRUE
    UNION ALL SELECT 6, '2021-07-01', '2022-06-30', 'Beca Académica', 10, 'ACTA-006', '2021-07-10', 'RES-006', 'RES-2021-003', 'POL-2021-003', TRUE
    UNION ALL SELECT 7, '2022-01-01', '2022-12-31', 'Beca Deportiva', 15, 'ACTA-007', '2022-01-12', 'RES-007', 'RES-2022-001', 'POL-2022-001', TRUE
    UNION ALL SELECT 8, '2022-01-01', '2022-12-31', 'Beca Socioeconómica', 20, 'ACTA-008', '2022-01-18', 'RES-008', 'RES-2022-002', 'POL-2022-002', TRUE
    UNION ALL SELECT 9, '2022-07-01', '2023-06-30', 'Beca Académica', 10, 'ACTA-009', '2022-07-08', 'RES-009', 'RES-2022-003', 'POL-2022-003', TRUE
    UNION ALL SELECT 10, '2023-01-01', '2023-12-31', 'Beca Cultural', 5, 'ACTA-010', '2023-01-10', 'RES-010', 'RES-2023-001', 'POL-2023-001', TRUE
) t
LEFT JOIN descuentos d ON d.id_estudiante = t.id_estudiante
WHERE d.id IS NULL;

-- 10. BECAS (Depende de estudiante). Solo id_estudiante 1 a 10 (persona_id).
-- Usa LEFT JOIN para omitir filas que ya existen (una beca por estudiante en datos de prueba).
INSERT INTO becas (id_estudiante, dedicacion, entidad_asociada, es_ofrecida_por_unicauca, tipo, titulo)
SELECT t.id_estudiante, t.dedicacion, t.entidad_asociada, t.es_ofrecida_por_unicauca, t.tipo, t.titulo
FROM (
    SELECT 1 AS id_estudiante, 'Tiempo Completo' AS dedicacion, 'Colciencias' AS entidad_asociada, 'No' AS es_ofrecida_por_unicauca, 'Beca Nacional' AS tipo, 'Beca de Excelencia Académica' AS titulo
    UNION ALL SELECT 2, 'Medio Tiempo', 'Universidad del Cauca', 'Sí', 'Beca Institucional', 'Beca de Investigación'
    UNION ALL SELECT 3, 'Tiempo Completo', 'ICETEX', 'No', 'Beca Nacional', 'Beca de Postgrado'
    UNION ALL SELECT 4, 'Medio Tiempo', 'Colciencias', 'No', 'Beca Nacional', 'Beca Jóvenes Investigadores'
    UNION ALL SELECT 5, 'Tiempo Completo', 'Universidad del Cauca', 'Sí', 'Beca Institucional', 'Beca de Excelencia'
    UNION ALL SELECT 6, 'Medio Tiempo', 'ICETEX', 'No', 'Beca Nacional', 'Beca de Postgrado'
    UNION ALL SELECT 7, 'Tiempo Completo', 'Colciencias', 'No', 'Beca Nacional', 'Beca de Investigación'
    UNION ALL SELECT 8, 'Medio Tiempo', 'Universidad del Cauca', 'Sí', 'Beca Institucional', 'Beca de Excelencia Académica'
    UNION ALL SELECT 9, 'Tiempo Completo', 'ICETEX', 'No', 'Beca Nacional', 'Beca de Postgrado'
    UNION ALL SELECT 10, 'Medio Tiempo', 'Colciencias', 'No', 'Beca Nacional', 'Beca Jóvenes Investigadores'
) t
LEFT JOIN becas b ON b.id_estudiante = t.id_estudiante
WHERE b.id IS NULL;
...
-- Limpiar duplicados en reporte_por_grupos (mantener solo el id mas bajo por combinacion)
DELETE rpg1 FROM reporte_por_grupos rpg1
INNER JOIN reporte_por_grupos rpg2
ON rpg1.configuracion_reporte_grupos_id = rpg2.configuracion_reporte_grupos_id
AND rpg1.grupo_id = rpg2.grupo_id
AND rpg1.id > rpg2.id;

-- Agregar UNIQUE constraint si no existe (para que INSERT IGNORE funcione)
SET @uk_rpg_exists = (SELECT COUNT(*) FROM information_schema.TABLE_CONSTRAINTS
    WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'reporte_por_grupos'
    AND CONSTRAINT_TYPE = 'UNIQUE' AND CONSTRAINT_NAME = 'uk_reporte_config_grupo');
SET @sql_rpg = IF(@uk_rpg_exists = 0,
    'ALTER TABLE reporte_por_grupos ADD UNIQUE KEY uk_reporte_config_grupo (configuracion_reporte_grupos_id, grupo_id)',
    'SELECT 1');
PREPARE stmt_rpg FROM @sql_rpg;
EXECUTE stmt_rpg;
DEALLOCATE PREPARE stmt_rpg;
...
-- UNIQUE KEY uk_reporte_config_grupo ya se agrega condicionalmente en la seccion 12 (antes de INSERT IGNORE)

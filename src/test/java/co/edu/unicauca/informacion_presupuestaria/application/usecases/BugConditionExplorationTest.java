...
class BugConditionExplorationTest {
    ...
    @Test
    void shouldHandleMixedRealAndSimulatedDataCorrectly() {
        // Arrange
        ...
        BecaDescuentoInfo becaReal = new BecaDescuentoInfo("BECA", 0.5f, "RES-01", "PAGADA", "SI");
        Student est = buildEstudiante("EST001", 2, List.of(becaReal));
        ...
        when(calculationService.calcular(any(), any(), any()))
                .thenReturn(new FinancialCalculationService.Totales(
                        new BigDecimal("2600000.00"), new BigDecimal("1300000.00"), new BigDecimal("1300000.00"), BigDecimal.ZERO));

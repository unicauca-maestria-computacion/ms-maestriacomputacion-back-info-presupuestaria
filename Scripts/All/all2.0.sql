...
-- =============================================================================
-- ALL 2.0 - Datos de prueba completos para reporte financiero y reporte por grupos
-- =============================================================================
-- Escenario:
--   * 2025-1 cerrado
--   * 2025-2 cerrado
--   * 2026-1 activo
--
-- Este bloque complementa el all.sql original con los datos que necesita el
-- modulo de informacion presupuestaria para calcular correctamente:
...
-- ---------------------------------------------------------------------------
-- 1. Parches idempotentes de esquema para ejecuciones locales
-- ---------------------------------------------------------------------------
DROP PROCEDURE IF EXISTS patch_all20_schema;
DELIMITER //
CREATE PROCEDURE patch_all20_schema()
BEGIN
    DECLARE CONTINUE HANDLER FOR SQLEXCEPTION BEGIN END;
    ...
    ALTER TABLE solicitudes ADD COLUMN radicado VARCHAR(50) NULL;
    ALTER TABLE solicitudes MODIFY COLUMN radicado VARCHAR(50) NULL;

    ALTER TABLE solicitudes_en_concejo ADD COLUMN porcentaje DECIMAL(5,2) NULL;
    ALTER TABLE solicitudes_en_concejo ADD COLUMN resolucion VARCHAR(100) NULL;
    ALTER TABLE solicitudes_en_concejo ADD COLUMN fecha_inicio DATE NULL;
    ALTER TABLE solicitudes_en_concejo ADD COLUMN fecha_fin DATE NULL;

    ALTER TABLE configuracion_reporte_financiero
        MODIFY COLUMN porcentaje_votacion_fijo DECIMAL(5,4) DEFAULT 0.1000;
    ALTER TABLE configuracion_reporte_financiero
        MODIFY COLUMN porcentaje_egresado_fijo DECIMAL(5,4) DEFAULT 0.0500;
END //
DELIMITER ;
CALL patch_all20_schema();
DROP PROCEDURE IF EXISTS patch_all20_schema;

-- ---------------------------------------------------------------------------
-- 2. Periodos canonicos del escenario de prueba
-- ---------------------------------------------------------------------------
INSERT INTO periodo_academico
    (id, tag_periodo, fecha_inicio, fecha_fin, fecha_fin_matricula, descripcion, estado)
VALUES
    (3, 1, '2025-02-01', '2025-06-30', '2025-02-15', 'Primer Periodo 2025', 'CERRADO'),
    (4, 2, '2025-08-01', '2025-12-15', '2025-08-15', 'Segundo Periodo 2025', 'CERRADO'),
    (5, 1, '2026-02-01', '2026-06-30', '2026-02-15', 'Primer Periodo 2026', 'ACTIVO')
ON DUPLICATE KEY UPDATE
    tag_periodo = VALUES(tag_periodo),
...
-- ---------------------------------------------------------------------------
-- 3. Grupos de investigacion base
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- 4. Configuracion financiera por periodo
-- ---------------------------------------------------------------------------
...
VALUES
    (3, 1423500.00,     0.00,     0.00, 1, 0.1000, 0.0500),
    (4, 1423500.00,     0.00,     0.00, 1, 0.1000, 0.0500),
    (5, 1623500.00, 57200.00, 57200.00, 0, 0.1000, 0.0500)
ON DUPLICATE KEY UPDATE
...
-- ---------------------------------------------------------------------------
-- 5. Configuracion presupuestal por grupos para cada periodo
-- ---------------------------------------------------------------------------
...
VALUES
    ...
    (5, 0.2200, 12000000.00, 0.4000, 0.6000, 0.0500)
ON DUPLICATE KEY UPDATE
...
SET @cfg_2025_1 = (SELECT id FROM configuracion_reporte_grupos WHERE periodo_academico_id = 3 LIMIT 1);
SET @cfg_2025_2 = (SELECT id FROM configuracion_reporte_grupos WHERE periodo_academico_id = 4 LIMIT 1);
SET @cfg_2026_1 = (SELECT id FROM configuracion_reporte_grupos WHERE periodo_academico_id = 5 LIMIT 1);

-- ---------------------------------------------------------------------------
-- 6. Participaciones iniciales por grupo
-- El backend recalcula estas participaciones dinamicamente con base en ingresos,
-- pero se dejan llenas para que la configuracion este completa desde BD.
-- ---------------------------------------------------------------------------
INSERT INTO participacion_grupo
    (configuracion_reporte_grupos_id, grupo_id, porcentaje_participacion,
     porcentaje_primer_semestre, porcentaje_segundo_semestre, vigencias_anteriores)
VALUES
    (@cfg_2025_1, @grupo_gti,  0.3400, 0.3400, 0.3400,       0.00),
    (@cfg_2025_1, @grupo_idis, 0.3300, 0.3300, 0.3300,       0.00),
    (@cfg_2025_1, @grupo_gico, 0.3300, 0.3300, 0.3300,       0.00),
    (@cfg_2025_2, @grupo_gti,  0.3500, 0.3500, 0.3500, 2500000.00),
    (@cfg_2025_2, @grupo_idis, 0.3300, 0.3300, 0.3300, 1800000.00),
    (@cfg_2025_2, @grupo_gico, 0.3200, 0.3200, 0.3200, 1600000.00),
    (@cfg_2026_1, @grupo_gti,  0.3600, 0.3600, 0.3600, 3100000.00),
    (@cfg_2026_1, @grupo_idis, 0.3400, 0.3400, 0.3400, 2400000.00),
    (@cfg_2026_1, @grupo_gico, 0.3000, 0.3000, 0.3000, 2100000.00)
ON DUPLICATE KEY UPDATE
...
-- ---------------------------------------------------------------------------
-- 7. Gastos generales por configuracion
-- ---------------------------------------------------------------------------
DELETE FROM gasto_general
WHERE configuracion_reporte_grupos_id IN (@cfg_2025_1, @cfg_2025_2, @cfg_2026_1);

INSERT INTO gasto_general
    (configuracion_reporte_grupos_id, categoria, descripcion, monto)
VALUES
    (@cfg_2025_1, 'Academico', 'Gestion de la Tecnologia', 8218800.00),
    (@cfg_2025_1, 'Academico', 'Competencias empresariales', 4799779.20),
    (@cfg_2025_1, 'Administracion', 'Gastos varios', 2000000.00),
    (@cfg_2025_1, 'Bienestar', 'Actividad con estudiantes o egresados', 3000000.00),
    (@cfg_2025_1, 'Bienestar', 'Apoyo estudiantes para segunda lengua', 1500000.00),
    (@cfg_2025_1, 'Monitores', 'Contratacion de monitores de apoyo al programa de pregrado o de posgrado', 2100000.00),
    (@cfg_2025_1, 'Contratacion', 'Contrato OPS Secretaria apoyo coordinacion Maestria, jefatura y programa ing. Sistemas', 27000000.00),
    (@cfg_2025_1, 'Contratacion', 'Aporte Contrato OPS Enlace FIET', 3445120.00),
    (@cfg_2025_1, 'Publicidad', 'Pago de elementos publicitarios para la Maestria en Computacion', 2000000.00),
    (@cfg_2025_1, 'Publicidad', 'Contrato OPS disenador publicidad para la Maestria en computacion', 3000000.00),
    (@cfg_2025_1, 'Administracion', 'Compra papeleria incluidos toners, kits de tinta', 1000000.00),
    (@cfg_2025_1, 'Eventos', 'Servicios de comida a la mesa para expertos invitados', 3000000.00),
    (@cfg_2025_1, 'Publicaciones', 'Publicaciones', 26000000.00),
    (@cfg_2025_1, 'Ajuste', 'Ajuste para total gastos generales 2025', 2499999.80),
    (@cfg_2026_1, 'Administracion', 'Papeleria, actas y archivo del periodo 2026-1', 1400000.00),
    (@cfg_2026_1, 'Infraestructura', 'Apoyo logistico laboratorios 2026-1', 2100000.00),
    (@cfg_2026_1, 'Eventos', 'Socializacion academica 2026-1', 1500000.00);

-- ---------------------------------------------------------------------------
-- 8. Matricula financiera: pago real y grupo asignado
-- ---------------------------------------------------------------------------
...
SELECT DISTINCT
    ...
    m.id_periodo,
    CASE
        WHEN m.id_periodo IN (3, 4) THEN 1
        WHEN m.id_periodo = 5 AND m.id_estudiante % 4 <> 0 THEN 1
        ELSE 0
    END AS esta_pago,
    CASE
        WHEN m.id_estudiante % 3 = 0 THEN @grupo_gti
        WHEN m.id_estudiante % 3 = 1 THEN @grupo_idis
        ELSE @grupo_gico
    END AS grupo_id
FROM matriculas m
WHERE m.id_periodo IN (3, 4, 5)
ON DUPLICATE KEY UPDATE
    esta_pago = VALUES(esta_pago),
    grupo_id = VALUES(grupo_id);

-- ---------------------------------------------------------------------------
-- 9. Proyecciones financieras completas para los periodos de prueba
-- ---------------------------------------------------------------------------
INSERT INTO proyeccion_estudiante
    (periodo_academico_id, estudiante_id, esta_pago, porcentaje_beca,
     aplica_votacion, aplica_egresado)
SELECT DISTINCT
    m.id_periodo,
    m.id_estudiante,
    COALESCE(mf.esta_pago, 0),
    CASE
        WHEN m.id_periodo IN (3, 4) AND m.id_estudiante IN (103, 106, 115) THEN 100.00
        WHEN m.id_periodo IN (3, 4) AND m.id_estudiante = 112 THEN 50.00
        WHEN m.id_periodo = 5 AND m.id_estudiante = 120 THEN 29.70
        WHEN m.id_periodo = 5 AND m.id_estudiante IN (121, 122, 126) THEN 25.00
        WHEN m.id_periodo = 5 AND m.id_estudiante = 125 THEN 22.50
        WHEN m.id_periodo = 5 AND m.id_estudiante IN (118, 123) THEN 4.50
        ELSE 0.00
    END AS porcentaje_beca,
    CASE
        WHEN m.id_estudiante IN (107, 112, 121, 122, 126) THEN 0
        ELSE 1
    END AS aplica_votacion,
    CASE
        WHEN m.id_estudiante IN (114, 119, 120, 121, 122, 123, 124, 125) THEN 1
        ELSE 0
    END AS aplica_egresado
FROM matriculas m
LEFT JOIN matricula_financiera mf
    ON mf.estudiante_id = m.id_estudiante
   AND mf.periodo_id = m.id_periodo
WHERE m.id_periodo IN (3, 4, 5)
ON DUPLICATE KEY UPDATE
    esta_pago = VALUES(esta_pago),
...
-- ---------------------------------------------------------------------------
-- 10. Escenario controlado 2025 para reporte por grupos
-- ---------------------------------------------------------------------------
-- Ingresos esperados:
--   2025-1: $75.775.740
--     GTI  50.44%, IDIS 28.93%, GICO 20.63%
--   2025-2: $70.507.740
--     GTI  46.74%, IDIS 31.09%, GICO 22.17%
--   Total ingresos matriculas 2025: $146.283.480
-- ---------------------------------------------------------------------------

SET @grupo_gti  = (SELECT id FROM grupo WHERE UPPER(TRIM(nombre)) = 'GTI' ORDER BY id LIMIT 1);
SET @grupo_idis = (SELECT id FROM grupo WHERE UPPER(TRIM(nombre)) = 'IDIS' ORDER BY id LIMIT 1);
SET @grupo_gico = (SELECT id FROM grupo WHERE UPPER(TRIM(nombre)) = 'GICO' ORDER BY id LIMIT 1);

UPDATE configuracion_reporte_financiero
SET valor_smlv = 12629290.00,
    porcentaje_votacion_fijo = 0.0000,
    porcentaje_egresado_fijo = 0.0000
WHERE periodo_academico_id = 3;

UPDATE configuracion_reporte_financiero
SET valor_smlv = 11751290.00,
    porcentaje_votacion_fijo = 0.0000,
    porcentaje_egresado_fijo = 0.0000
WHERE periodo_academico_id = 4;

-- Limpiar las asignaciones financieras 2025 antes de crear el escenario
-- controlado. Si quedaron filas previas del mismo estudiante/periodo, el
-- reporte puede tomar un grupo anterior aunque el pago real este en cero.
DELETE FROM matricula_financiera
WHERE periodo_id IN (3, 4);

UPDATE proyeccion_estudiante
SET esta_pago = 0,
    aplica_votacion = 0,
    aplica_egresado = 0,
    porcentaje_beca = 0.00
WHERE periodo_academico_id IN (3, 4);
...
DELETE dc
FROM documentos_concejo dc
JOIN solicitudes_en_concejo sec ON sec.id = dc.id_solicitud_concejo
WHERE sec.id_solicitud IN (310101, 310110, 310111, 310117, 310119, 310210, 310217, 310218);

DELETE FROM solicitudes_en_concejo
WHERE id_solicitud IN (310101, 310110, 310111, 310117, 310119, 310210, 310217, 310218);

DELETE FROM solicitudes_en_comite
WHERE id_solicitud IN (310101, 310110, 310111, 310117, 310119, 310210, 310217, 310218);

DELETE FROM solicitud_beca_descuento
WHERE id_solicitud IN (310101, 310110, 310111, 310117, 310119, 310210, 310217, 310218);

DELETE FROM solicitudes
WHERE id IN (310101, 310110, 310111, 310117, 310119, 310210, 310217, 310218);

INSERT INTO solicitudes (id, id_tipo_solicitud, id_estudiante, id_tutor, estado, radicado)
VALUES
     (310101, @id_tipo_beca, 101, 2, 'RESUELTA', 'ALL20-25-101'),
     (310111, @id_tipo_beca, 111, 2, 'RESUELTA', 'ALL20-25-111'),
     (310119, @id_tipo_beca, 119, 2, 'RESUELTA', 'ALL20-25-119'),
     (310210, @id_tipo_beca, 110, 2, 'RESUELTA', 'ALL20-25-210'),
     (310217, @id_tipo_beca, 117, 2, 'RESUELTA', 'ALL20-25-217'),
     (310218, @id_tipo_beca, 118, 2, 'RESUELTA', 'ALL20-25-218')
ON DUPLICATE KEY UPDATE
    estado = VALUES(estado),
...
INSERT IGNORE INTO solicitud_beca_descuento (id_solicitud, tipo, motivo)
VALUES
     (310101, 'Beca - Ajuste ALL 2.0', 'Ajuste participacion 2025-1 GTI'),
     (310111, 'Beca - Ajuste ALL 2.0', 'Ajuste participacion 2025-1 IDIS'),
     (310119, 'Beca - Ajuste ALL 2.0', 'Ajuste participacion 2025-1 GICO'),
     (310210, 'Beca - Ajuste ALL 2.0', 'Ajuste participacion 2025-2 GTI'),
     (310217, 'Beca - Ajuste ALL 2.0', 'Ajuste participacion 2025-2 IDIS'),
     (310218, 'Beca - Ajuste ALL 2.0', 'Ajuste participacion 2025-2 GICO');

INSERT IGNORE INTO solicitudes_en_comite (id_solicitud, avalado_comite, concepto_comite)
VALUES
     (310101, 'SI', 'Ajuste escenario ALL 2.0'),
     (310111, 'SI', 'Ajuste escenario ALL 2.0'),
     (310119, 'SI', 'Ajuste escenario ALL 2.0'),
     (310210, 'SI', 'Ajuste escenario ALL 2.0'),
     (310217, 'SI', 'Ajuste escenario ALL 2.0'),
     (310218, 'SI', 'Ajuste escenario ALL 2.0');

INSERT INTO solicitudes_en_concejo
    (id, id_solicitud, avalado_concejo, porcentaje, resolucion, fecha_inicio, fecha_fin)
VALUES
     (310101, 310101, 'SI', 49.57, 'ALL20-2025-1-GTI',  '2025-02-01', '2025-06-30'),
     (310111, 310111, 'SI', 71.07, 'ALL20-2025-1-IDIS', '2025-02-01', '2025-06-30'),
     (310119, 310119, 'SI', 79.36, 'ALL20-2025-1-GICO', '2025-02-01', '2025-06-30'),
     (310210, 310210, 'SI', 52.89, 'ALL20-2025-2-GTI',  '2025-08-01', '2025-12-15'),
     (310217, 310217, 'SI', 68.67, 'ALL20-2025-2-IDIS', '2025-08-01', '2025-12-15'),
     (310218, 310218, 'SI', 78.44, 'ALL20-2025-2-GICO', '2025-08-01', '2025-12-15')
ON DUPLICATE KEY UPDATE
    avalado_concejo = VALUES(avalado_concejo),
...
SET porcentaje_primer_semestre = 0.5044,
    porcentaje_segundo_semestre = 0.4674,
    porcentaje_participacion = 0.4859
WHERE configuracion_reporte_grupos_id IN (@cfg_2025_1, @cfg_2025_2)
  AND grupo_id = @grupo_gti;

UPDATE participacion_grupo
SET porcentaje_primer_semestre = 0.2893,
    porcentaje_segundo_semestre = 0.3109,
    porcentaje_participacion = 0.3001
WHERE configuracion_reporte_grupos_id IN (@cfg_2025_1, @cfg_2025_2)
  AND grupo_id = @grupo_idis;

UPDATE participacion_grupo
SET porcentaje_primer_semestre = 0.2063,
    porcentaje_segundo_semestre = 0.2217,
    porcentaje_participacion = 0.2140
WHERE configuracion_reporte_grupos_id IN (@cfg_2025_1, @cfg_2025_2)
  AND grupo_id = @grupo_gico;

-- ---------------------------------------------------------------------------
-- 11. Certificados de votacion aprobados para que matricula financiera tambien
--     pueda resolver aplicaVotacion desde solicitudes.
-- ---------------------------------------------------------------------------
INSERT INTO tipos_solicitudes (codigo, nombre)
SELECT 'CER_VOTO', 'Registro de certificado de votacion'
WHERE NOT EXISTS (
    SELECT 1 FROM tipos_solicitudes WHERE codigo = 'CER_VOTO'
);

SET @tipo_voto = (SELECT id FROM tipos_solicitudes WHERE codigo = 'CER_VOTO' LIMIT 1);

INSERT IGNORE INTO solicitudes
    (id, id_tipo_solicitud, id_estudiante, id_tutor, estado, radicado)
VALUES
    (300101, @tipo_voto, 101, 1, 'APROBADA', 'VOT101'),
    (300102, @tipo_voto, 102, 1, 'APROBADA', 'VOT102'),
    (300103, @tipo_voto, 103, 1, 'APROBADA', 'VOT103'),
    (300104, @tipo_voto, 104, 1, 'APROBADA', 'VOT104'),
    (300105, @tipo_voto, 105, 1, 'APROBADA', 'VOT105'),
    (300106, @tipo_voto, 106, 1, 'APROBADA', 'VOT106'),
    (300108, @tipo_voto, 108, 1, 'APROBADA', 'VOT108'),
    (300109, @tipo_voto, 109, 1, 'APROBADA', 'VOT109'),
    (300110, @tipo_voto, 110, 1, 'APROBADA', 'VOT110'),
    (300111, @tipo_voto, 111, 1, 'APROBADA', 'VOT111'),
    (300113, @tipo_voto, 113, 1, 'APROBADA', 'VOT113'),
    (300114, @tipo_voto, 114, 1, 'APROBADA', 'VOT114'),
    (300115, @tipo_voto, 115, 1, 'APROBADA', 'VOT115'),
    (300116, @tipo_voto, 116, 1, 'APROBADA', 'VOT116'),
    (300117, @tipo_voto, 117, 1, 'APROBADA', 'VOT117'),
    (300118, @tipo_voto, 118, 1, 'APROBADA', 'VOT118'),
    (300119, @tipo_voto, 119, 1, 'APROBADA', 'VOT119'),
    (300120, @tipo_voto, 120, 1, 'APROBADA', 'VOT120'),
    (300123, @tipo_voto, 123, 1, 'APROBADA', 'VOT123'),
    (300124, @tipo_voto, 124, 1, 'APROBADA', 'VOT124'),
    (300125, @tipo_voto, 125, 1, 'APROBADA', 'VOT125');

-- ---------------------------------------------------------------------------
-- 12. Resumen de control del escenario ALL 2.0
-- ---------------------------------------------------------------------------
SELECT
    'ALL 2.0 listo: 2025-1 y 2025-2 cerrados, 2026-1 activo, datos financieros y grupos completos.' AS resultado;
...
SELECT
    ...
    COUNT(DISTINCT pe.estudiante_id) AS proyecciones,
    COUNT(DISTINCT mf.estudiante_id) AS matriculas_financieras
FROM periodo_academico pa
LEFT JOIN matriculas m ON m.id_periodo = pa.id
LEFT JOIN proyeccion_estudiante pe ON pe.periodo_academico_id = pa.id
LEFT JOIN matricula_financiera mf ON mf.periodo_id = pa.id
WHERE pa.id IN (3, 4, 5)
GROUP BY pa.id, pa.descripcion, pa.estado
ORDER BY pa.id;
